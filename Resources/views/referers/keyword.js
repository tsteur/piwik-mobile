 ...
 * @version $Id$
 * 
 * @fileOverview View template referers/keyword.
 */
...
function template () {
    ...
    if (this.keyword && (this.keyword instanceof Array) && 0 < this.keyword.length) {
        for(var index = 0; index < this.keyword.length; index++) {
            keyword.push({title: this.keyword[index].label,
                          value: this.keyword[index][config.getUsedRow(this.period)]});
        }
    }
    
    var box            = this.helper('borderedContainer', {top: top});
    
    var headline       = this.helper('headline', {headline: _('Referers_Keywords')});
    ...
    var headlineStats  = {title: _('Referers_ColumnKeyword'), 
                          value: config.getUsedRowTranslation()};
                         
    var visitorStats   = this.helper('statisticList', {values: keyword, 
