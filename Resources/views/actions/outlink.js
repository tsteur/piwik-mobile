 ...
 * @version $Id$
 * 
 * @fileOverview View template actions/outlink.
 */
...
function template () {
    ...
    if (this.outlink && (this.outlink instanceof Array) && 0 < this.outlink.length) {
        for(var index = 0; index < this.outlink.length; index++) {
            outlink.push({title: this.outlink[index].label,
                          value: this.outlink[index][config.getUsedRow(this.period)]});
        }
    }
    
    var box           = this.helper('borderedContainer', {top: top});
    
    var headline      = this.helper('headline', {headline: _('Actions_SubmenuOutlinks')});
    ...
    var headlineStats = {title: _('Actions_ColumnClickedURL'), 
                         value: config.getUsedRowTranslation()};
                         
    var visitorStats  = this.helper('statisticList', {values: outlink, 
