 ...
 * @version $Id$
 * 
 * @fileOverview View template actions/pagetitle.
 */
...
function template () {
    ...
    if (this.pagetitle && (this.pagetitle instanceof Array) && 0 < this.pagetitle.length) {
        for(var index = 0; index < this.pagetitle.length; index++) {
            pagetitle.push({title: this.pagetitle[index].label,
                            value: this.pagetitle[index][config.getUsedRow(this.period)]});
        }
    }
    
    var box           = this.helper('borderedContainer', {top: top});
    
    var headline      = this.helper('headline', {headline: _('Actions_SubmenuPageTitles')});
    ...
    var headlineStats = {title: _('Actions_ColumnPageName'), 
                         value: config.getUsedRowTranslation()};
                         
    var visitorStats  = this.helper('statisticList', {values: pagetitle, 
