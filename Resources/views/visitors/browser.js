 ...
 * @version $Id$
 * 
 * @fileOverview View template visitors/browser.
 */
...
function template () {
    ...
    if (this.browsers && (this.browsers instanceof Array) && 0 < this.browsers.length) {
    ...
    var headline      = this.helper('headline', {headline: _('UserSettings_Browsers')});
    ...
    if (this.graphsEnabled) {
        ...
        if (this.browserTypes && (this.browserTypes instanceof Array) && 0 < this.browserTypes.length) {
            for(var index = 0; index < this.browserTypes.length; index++) {
                browserTypes[this.browserTypes[index].shortLabel] = this.browserTypes[index][config.getUsedRow(this.period)];
            }
        }
        
        var graphUrl   = Graph.getPieChartUrl(browserTypes);
        
        var graph      = this.helper('graph', {title: _('UserSettings_BrowserFamilies'),
                                               graphUrl: graphUrl,
                                               top: top});
    ...
    var headlineStats  = {title: _('UserSettings_Browsers'), 
                          value: config.getUsedRowTranslation()};
                         
    var visitorStats   = this.helper('statisticList', {values: browsers, 
