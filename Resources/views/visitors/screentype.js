 ...
 * @version $Id$
 * 
 * @fileOverview View template visitors/screentype.
 */
...
function template () {
    ...
    var headline       = this.helper('headline', {headline: _('UserSettings_WideScreen')});
    ...
    if (this.graphsEnabled) {
        ...
        if (this.screentype && (this.screentype instanceof Array) && 0 < this.screentype.length) {
            ...
            for (var index = 0; index < this.screentype.length; index++) {
                screentypePie[this.screentype[index].label] = this.screentype[index][config.getUsedRow(this.period)];
            }
        }
        
        var graphUrl   = Graph.getPieChartUrl(screentypePie);
        
        var graph      = this.helper('graph', {title: _('UserSettings_WideScreen'),
                                               graphUrl: graphUrl,
                                               top: top});
    ...
    var headlineStats  = {title: _('UserSettings_ColumnTypeOfScreen'), 
                          value: config.getUsedRowTranslation()};
                         
    var visitorStats   = this.helper('statisticList', {values: screentype, 
