 ...
 * @version $Id$
 * 
 * @fileOverview View template visitors/continent.
 */
...
function template () {
    ...
    if (this.continents && (this.continents instanceof Array) && 0 < this.continents.length) {
        for (var index = 0; index < this.continents.length; index++) {
        	continents.push({title: this.continents[index].label,
                             value: this.continents[index][config.getUsedRow(this.period)]});
        }
    }
    
    var box            = this.helper('borderedContainer', {top: top});
    
    var headline       = this.helper('headline', {headline: _('UserCountry_WidgetContinents')});
    ...
    if (this.graphsEnabled) {
        ...
        if (this.continents && (this.continents instanceof Array) && 0 < this.continents.length) {
            ...
            for (var index = 0; index < this.continents.length; index++) {
            	continentsPie[this.continents[index].label] = this.continents[index][config.getUsedRow(this.period)];
            }
        }
        
        var graphUrl   = Graph.getPieChartUrl(continentsPie);
        
        var graph      = this.helper('graph', {title: _('UserCountry_Continent'),
                                               graphUrl: graphUrl,
                                               top: top});
    ...
    var headlineStats  = {title: _('UserCountry_Continent'), 
                          value: config.getUsedRowTranslation()};
                         
    var visitorStats   = this.helper('statisticList', {values: continents, 
