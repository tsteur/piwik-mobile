 ...
 * @version $Id$
 * 
 * @fileOverview View template visitors/resolution.
 */
...
function template () {
    ...
    if (this.resolution && (this.resolution instanceof Array) && 0 < this.resolution.length) {
        for(var index = 0; index < this.resolution.length; index++) {
            resolution.push({title: this.resolution[index].label,
                             value: this.resolution[index][config.getUsedRow(this.period)]});
        }
    }
    
    var box           = this.helper('borderedContainer', {top: top});
    
    var headline      = this.helper('headline', {headline: _('UserSettings_Resolutions')});
    ...
    if (this.graphsEnabled) {
        ...
        if (this.resolution && (this.resolution instanceof Array) && 0 < this.resolution.length) {
            for(var index = 0; index < this.resolution.length; index++) {
                resolutionPie[this.resolution[index].label] = this.resolution[index][config.getUsedRow(this.period)];
            }
        }
        
        var graphUrl   = Graph.getPieChartUrl(resolutionPie);
        
        var graph      = this.helper('graph', {title: _('UserSettings_Resolutions'),
                                               graphUrl: graphUrl,
                                               top: top});
    ...
    var headlineStats  = {title: _('UserSettings_ColumnResolution'), 
                          value: config.getUsedRowTranslation()};
                         
    var visitorStats   = this.helper('statisticList', {values: resolution, 
