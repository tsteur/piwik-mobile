 ...
 * @version $Id$
 * 
 * @fileOverview View template visitors/os.
 */
...
function template () {
    ...
    var headline      = this.helper('headline', {headline: _('UserSettings_OperatingSystems')});
    ...
    if (this.graphsEnabled) {
        ...
        if (this.os && (this.os instanceof Array) && 0 < this.os.length) {
            for(var index = 0; index < this.os.length; index++) {
                osPie[this.os[index].label] = this.os[index][config.getUsedRow(this.period)];
            }
        }
        
        var graphUrl   = Graph.getPieChartUrl(osPie);
        
        var graph      = this.helper('graph', {title: _('UserSettings_OperatingSystems'),
                                                  graphUrl: graphUrl,
                                                  top: top});
    ...
    var headlineStats  = {title: _('UserSettings_ColumnOperatingSystem'), 
                          value: config.getUsedRowTranslation()};
                         
    var visitorStats   = this.helper('statisticList', {values: os, 
