...
Piwik.Network.LiveRequest = function () {
     ...
     */
    this.onReceiveLiveMinutes = function (response) {
        if (!response || !(response instanceof Array) || !response.length) {
            Piwik.Log.warn('Response is not valid', 'Piwik.Network.LiveRequest::onReceiveLiveMinutes');

            return;
        }

        if (!response[0] || 'undefined' == typeof response[0].actions) {
            Piwik.Log.warn('No values given', 'Piwik.Network.LiveRequest::onReceiveLiveMinutes');
     ...
     */
    this.onReceiveLiveHours = function (response) {
        if (!response || !(response instanceof Array) || !response.length) {
            Piwik.Log.warn('Response is not valid', 'Piwik.Network.LiveRequest::onReceiveLiveHours');

            return;
        }

        if (!response[0] || 'undefined' == typeof response[0].actions) {
            Piwik.Log.warn('No values given', 'Piwik.Network.LiveRequest::onReceiveLiveHours');
     ...
     */
    this.onReceiveLiveDetails = function (response) {
        if (!response || !(response instanceof Array)) {
            Piwik.Log.warn('Response is not valid', 'Piwik.Network.LiveRequest::onReceiveLiveDetails');
