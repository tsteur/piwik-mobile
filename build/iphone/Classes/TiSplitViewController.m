...
{
	if (self = [super init]) {
		_piwikmobileRoot = [rootController retain];
		proxy = split_;
...
-(void)dealloc
{
	RELEASE_TO_NIL(_piwikmobileRoot);
...
-(void)loadView
{
	[[NSNotificationCenter defaultCenter] removeObserver:_piwikmobileRoot name:UIDeviceOrientationDidChangeNotification object:nil];
	[super loadView];
}
...
{
	[_piwikmobileRoot refreshOrientationModesIfNeeded:oldCurrentWindow];
}
