...
@implementation PiwikMobileObject
...
-(void)injectPatches
{
	// called to inject any PiwikMobile patches in JS before a context is loaded... nice for 
...
{
	// create PiwikMobile global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_piwikmobile = [[PiwikMobileObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_piwikmobile];
	
	NSString *_piwikmobileNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_piwikmobileNS UTF8String]);
	...
	if (preload!=nil)
	{
		KrollObject *ti = (KrollObject*)[_piwikmobile valueForKey:@"UI"];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.piwikmobile.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
